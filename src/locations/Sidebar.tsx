...
const Sidebar = () => {
  ...
  const handleOpenProductPicker = async () => {
    if (!parameters.storeHash || !parameters.accessToken) {
      sdk.notifier.error(
        'Please configure BigCommerce credentials in the app configuration'
      );
      return;
    }
  ...
  const handleSelectVariant = async (
    ...
    try {
      ...
      if (!contentField) {
        sdk.notifier.error(
          'No content field found. Please ensure your content type has a rich text field named "content".'
        );
        return;
      }
      ...
      const productLink = {
        nodeType: 'hyperlink',
        data: {
          uri: `bc-product://${variant.sku}?data=${encodeURIComponent(
        ...
        content: [
          {
            nodeType: 'text',
            value: `🛒 ${product.name} - SKU: ${variant.sku} - £${(
              variant.calculated_price || variant.price
            ).toFixed(2)}`,
            marks: [],
            data: {},
          },
  ...
  return (
    <Box padding='none'>
      <Stack spacing='spacingS' padding='none' flexDirection='column'>
        <Button variant='primary' onClick={handleOpenProductPicker} isFullWidth>
          Browse products...
        </Button>
      </Stack>
    </Box>
